...
applies_when:
  ...
  - writing or revising CLAUDE.md, AGENTS.md, or project context
...
- **Encode invariants up-front.** A project context file (e.g., `CLAUDE.md` or `AGENTS.md`) should declare: architectural boundaries, forbidden data shapes (no positional arrays, no untyped maps for structured data), concurrency rules (who may mutate render-visible state), and explicit non-goals.
